/* waifu2x-cpu_kai Ver.1.0 by YSR */
/* rgb2ycとyc2rgbの使い方が分からない…… */
...
#define kSoftName "waifu2x-cpu_kai[SSE]"
...
FILTER_DLL filter = {
	...
	NULL, NULL,
	NULL, NULL,
	"waifu2x-cpu_kai version 1.0 by YSR",
	NULL, NULL,
};
// 1ステップにおけるデータ
struct Step{
...
BOOL func_init(FILTER *fp) {
	try {
		...
		g_model_data[kModelScale2x].Init(".\\plugins\\models\\scale2.0x_model_kai.dat");
	}
	...
	try {
		...
		if(!fp->exfunc->is_saving(fpip->editp)) {
			if((fp->track[kTrackNoise] == 0) && (fp->track[kTrackScale] == 0)) {
				...
				// 計算する場合
				SetWindowText(fp->hwnd, _T("waifu2x-cpu_kai(処理中...)"));
			}
		}
		...
		if(!fp->exfunc->is_saving(fpip->editp)) {
			...
			title_bar << "waifu2x-cpu_kai(" << ms << "ms)";
				...
				for(auto y = 0; y < input_size_y_; ++y) {
					for(auto x = 0, x_SIMD = 0, x_ = 0; x < input_size_x; x += SIMD, ++x_SIMD, x_ += kMaxOutput) {
						for(auto o = 0; o < output_plane_size; ++o) {
							...
							// 負数のみ0.1倍する
							PackedFloat ge_zero = PackedAnd(PackedCmpGe(temp_simd, kZeroSIMD), temp_simd);	//0.0以上の成分
							PackedFloat lt_zero = PackedAnd(PackedCmpLt(temp_simd, kZeroSIMD), temp_simd);	//0.0未満の成分
							output_picture_rgb[y][x_ + o] = PackedOr(ge_zero, PackedMul(lt_zero, kConstSIMD));
						}
					}
				}
				// 出力平面を入力平面に反映する
#pragma omp parallel for num_threads(thread_)
	...
	/* 元通り書き戻す */
	//filter.exfunc->yc2rgb(pixelp, fpip->ycp_edit, fpip->w);
	return;
}
